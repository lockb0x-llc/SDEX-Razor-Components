...
async function fetchOpenOrders() {
    try {
        ...
        orders.forEach(order => {
            // Calculate reciprocal price if needed
            const formattedPrice = parseFloat(order.price).toFixed(7); // Standard price format
            const reciprocalPrice = (1 / parseFloat(order.price)).toFixed(7); // Reciprocal price

            const isSellingNative = order.selling.asset_type === "native";
            const isBuyingNative = order.buying.asset_type === "native";

            // Decide which price to show based on selling/buying assets
